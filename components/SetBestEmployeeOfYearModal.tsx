...
interface SetBestEmployeeOfYearModalProps {
  users: User[];
  onClose: () => void;
...
export const SetBestEmployeeOfYearModal: React.FC<SetBestEmployeeOfYearModalProps> = ({ users, onClose, onSet, selectedIds }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-lg p-8 w-full max-w-md shadow-xl">
        <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Set Employee of the Year</h2>
            <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-2xl">&times;</button>
        </div>
        <p className="text-slate-600 dark:text-slate-300 mb-6">Select up to two employees for their exceptional contributions this year.</p>
        ...
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <button 
            ...
            className="px-4 py-2 bg-slate-600 text-white rounded-lg hover:bg-slate-700 transition disabled:bg-slate-400 disabled:cursor-not-allowed"
          >
            Set Employee(s)
          </button>
        </div>
